...
WHERE milliseconds >
	(SELECT AVG(milliseconds) AS avg_song_ms
	 FROM Track)
...
WITH best_selling_artist AS(
	...
	SUM(invoice_line.unit_price*invoice_line.quantity) AS total_amountSpent FROM invoice_line
	JOIN track ON track.track_id = invoice_line.track_id
	JOIN album ON album.album_id = track.album_id
	JOIN artist ON artist.artist_id = album.artist_id
	GROUP BY 1 --1=artist_id, 2=artist_name, 3=total_sales
	ORDER BY 3 DESC
	LIMIT 1
 )
 
 SELECT c.customer_id, c.first_name, c.last_name, bsa.artist_name,
 SUM(il.unit_price*il.quantity) AS total_amountSpent
 FROM invoice i
 ...
 GROUP BY 1,2,3,4 --1=customer_id, 2=first_name, 3=last_name, 4=artist.name, 5=total_amountSpent
 ORDER BY 5 DESC
